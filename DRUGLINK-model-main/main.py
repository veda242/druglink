...
import shelve
import traceback
from drug_validator import is_valid_drug
import re
from fastapi import HTTPException
from drug_validator import VALID_DRUGS
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],        # DEV: allow all origins (replace with exact origin(s) in production)
...
def call_llama_with_bullets(prompt, text, label, conf, dosage_summary):
    ...
    # If Groq client not available -> fallback
    if not GROQ_AVAILABLE:
        fb = generate_bullet_fallback(text, label, conf, dosage_summary)
        cache_set(key, fb)
        return f"[fallback: client missing] {fb}"

    api_key = os.environ.get("GROQ_API_KEY")
    if not api_key:
        fb = generate_bullet_fallback(text, label, conf, dosage_summary)
        cache_set(key, fb)
        return f"[fallback: no API key] {fb}"

    try:
        client = Groq(api_key=api_key)
        # Build a clear instruction asking for bullet points only
        system_msg = (
        ...
        resp = client.chat.completions.create(
            model="llama3-8b-8192",  # change model if desired (llama3-70b-8192 for larger)
            messages=[
        ...
        # Cache the successful LLaMA result
        cache_set(key, msg)
        return msg
    except Exception as e:
        fb = generate_bullet_fallback(text, label, conf, dosage_summary)
        cache_set(key, fb)
        return f"[fallback: error {e}] {fb}"
    
# -----------------------

def get_candidate_drug_words(text: str):
    """
    Very simple: take all alphabetic words except common filler words.
    """
    words = re.findall(r"[a-zA-Z]+", text.lower())
    blacklist = {"with", "and", "or", "at", "in", "for", "of", "the", "as", "sample", "drug", "drugs"}
    return [w for w in words if w not in blacklist]


@app.post("/predict_enhanced")
def predict_enhanced(q: Query):
    texts = q.texts
    if not texts:
        raise HTTPException(status_code=400, detail="No texts provided")

    # take the first text (what your frontend sends)
    text = texts[0]

    # 1️⃣ Get candidate words that might be drug names
    candidates = get_candidate_drug_words(text)
    print("Candidates:", candidates)  # debug

    # 2️⃣ Anything not in VALID_DRUGS is invalid
    invalid = [w for w in candidates if w not in VALID_DRUGS]
    print("Invalid:", invalid)  # debug

    if invalid:
        invalid_list = ", ".join(invalid)
        raise HTTPException(
            status_code=400,
            detail=f"The following are not valid drug names: {invalid_list}. Please enter valid drug names only."
        )


    # 3️⃣ If we reached here, all names are valid → run model
    try:
    ...
    # ... rest of your existing code that builds the response
    ...
    for i, txt in enumerate(texts):
        ...
        # Prompt: ask for bullet points and a fixed structure
        prompt = (
            f"Input: \"{txt}\".\n"
